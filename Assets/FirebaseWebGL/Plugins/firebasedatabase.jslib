mergeInto(LibraryManager.library, {
    ...
    StopListeningForValueChanged: function(path, parsedObjectName, callback, fallback) {
        ...
        try {
            ...
            window.unityInstance.Module.SendMessage(parsedObjectName, parsedCallback, "Success: listener removed");
        } catch (error) {
            window.unityInstance.Module.SendMessage(parsedObjectName, parsedFallback, JSON.stringify(error, Object.getOwnPropertyNames(error)));
        }
    },
    ...
    ModifyNumberWithTransaction: function(path, amount, objectName, callback, fallback) {
        ...
        try {
            ...
            firebase.database().ref(parsedPath).transaction(function(currentValue) {
                ...
                window.unityInstance.Module.SendMessage(parsedObjectName, parsedCallback, "Success: transaction run in " + parsedPath);
            });
